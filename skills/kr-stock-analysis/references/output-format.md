...
16. Structured Stance
   State the current stance, why the memo stops there, and what would change the view. Use language such as `관심 유지`, `추가 검증 전 보류`, `핵심 리스크 해소 시 재평가`, or `지금은 피하는 쪽` instead of a forced buy or sell call unless the user explicitly asked for one.
17. Follow-up Research Prompts
