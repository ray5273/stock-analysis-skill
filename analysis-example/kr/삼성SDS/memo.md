...
위 세 이미지는 `2026-04-05`에 재생성했고, 데이터 기준은 `2026-04-03`까지의 1년 일봉이다. PNG 상단 제목에는 종목명 `삼성SDS`가 직접 표시되도록 생성했다. 첫 번째 이미지는 `캔들스틱`, `종가선`, `5/20/60/120일선`, `거래량`을, 두 번째 이미지는 `종가선`, `볼린저밴드`, `일목균형표`, `RSI14`를, 세 번째 이미지는 `MACD`, `시그널`, `히스토그램`, `ADX/DMI`를 보여준다.

차트 지표는 `종가 152,200원`, `5일선 152,440원`, `20일선 158,200원`, `60일선 169,092원`, `120일선 170,217원`, `볼린저 상단 167,536원`, `볼린저 하단 148,864원`, `전환선 155,250원`, `기준선 173,100원`, `현재 구름대 A 174,225원`, `현재 구름대 B 174,150원`, `미래 구름대 A 164,175원`, `미래 구름대 B 173,100원`, `RSI14 44.30`, `MACD -4,581`, `시그널 -4,193`, `히스토그램 -388`, `ADX14 23.32`, `+DI 16.66`, `-DI 31.21`, `거래량/20일 평균 60.2%`, `20일 돌파 레벨 168,500원`, `20일 이탈 레벨 149,300원`이었다.

해석은 아직 `약세 지속` 쪽이다. 현재 주가는 `5일선`, `20일선`, `60일선`, `120일선` 아래에 있고, 일목균형표 기준으로도 `below-cloud`다. RSI는 `44.30`으로 과매도는 아니지만 추세 전환 신호도 아니다. `MACD` 역시 `시그널` 아래이면서 `0선` 밑에 있어 반등보다 약세 구조가 우위다.

다만 `히스토그램 축소`는 하락 모멘텀이 더 강해지는 국면은 아니라는 뜻이다. 반대로 `ADX 23.32`와 `-DI 우위`는 약세 추세 강도가 서서히 살아나는 구조라는 점을 보여준다. 아래로는 `149,300원` 이탈 여부가 가장 중요하고, 위로는 `155,250원` 전환선, `158,200원` 20일선, 그 다음 `168,500원` 돌파 레벨 회복이 필요하다. 차트만 놓고 보면 지금은 `선행 매수보다 확인 매수`가 더 맞는 자리다.
