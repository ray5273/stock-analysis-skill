...
아래 표에서 `현재가`와 `시가총액`은 `2026-04-09` 종가 기준이다. `Trailing PER`와 `P/B`는 `2025년 연간 EPS/BPS`를 기준으로 다시 계산했고, 발행주식수는 `77,377,800주`를 사용했다.

| 항목 | 값 | 기준일 | 출처 | 비고 |
| --- | --- | --- | --- | --- |
| 현재가 | `148,300원` | 2026-04-09 | 1년 일봉 데이터 | Yahoo Finance KRX 일봉 기준 최신 종가 |
| 발행주식수 | `77,377,800주` | 2025-12-31 기준 | DART / FnGuide | 보통주 기준 |
| 시가총액 | `약 11.48조원` | 2026-04-09 | 현재가 × 발행주식수 | 파생 |
| Trailing PER | `약 15.10배` | 2026-04-09 | 현재가 / 2025 EPS | EPS `9,819원` 기준 |
| Forward PER | `not separately disclosed` |  |  | 현재 source set 기준 |
| P/B | `약 1.16배` | 2026-04-09 | 현재가 / 2025 BPS | BPS `약 128,152원` 기준 |
...
| 배당수익률 | `약 2.15%` | 2026-04-09 | DPS / 현재가 | 파생 |

절대적으로 비싸다고 보긴 어렵다. 다만 `AI/클라우드 pure-play` 프리미엄을 받기엔 물류 비중이 너무 크고, 반대로 전통적 SI주로만 보기엔 클라우드 질적 변화가 진행 중이다. `2026-04-09` 현재 trailing `PER 15.1배`, `P/B 1.16배`는 2025년 말 기준보다는 한 단계 낮아졌고, 최근 5년 밴드 안에서는 `중하단` 쪽으로 내려온 상태다.
...
- `PER` 5년 범위는 `8.65배~19.81배`였다. `2025년 말 17.47배`는 범위 중상단이지만, `2026-04-09` 현재 trailing `15.10배`는 다시 중간 아래로 내려왔다.
- `P/B` 5년 범위는 `1.06배~1.67배`였다. `2026-04-09` 현재 `1.16배`는 `2022년 말`, `2024년 말`에 가까운 낮은 구간이다.
...
위 세 이미지는 `2026-04-09`에 재생성했고, 데이터 기준은 `2026-04-09`까지의 1년 일봉이다. PNG 상단 제목에는 종목명 `삼성SDS`가 직접 표시되도록 생성했다. 첫 번째 이미지는 `캔들스틱`, `종가선`, `5/20/60/120일선`, `거래량`을, 두 번째 이미지는 `종가선`, `볼린저밴드`, `일목균형표`, `RSI14`를, 세 번째 이미지는 `MACD`, `시그널`, `히스토그램`, `ADX/DMI`를 보여준다.

차트 지표는 `종가 148,300원`, `5일선 151,640원`, `20일선 156,170원`, `60일선 167,408원`, `120일선 169,688원`, `볼린저 상단 166,060원`, `볼린저 하단 146,280원`, `전환선 153,350원`, `기준선 159,300원`, `현재 구름대 A 176,050원`, `현재 구름대 B 176,050원`, `미래 구름대 A 156,325원`, `미래 구름대 B 172,600원`, `RSI14 35.61`, `MACD -4,364`, `시그널 -4,292`, `히스토그램 -72`, `ADX14 23.39`, `+DI 19.24`, `-DI 31.08`, `거래량/20일 평균 255.2%`, `20일 돌파 레벨 168,200원`, `20일 이탈 레벨 149,300원`이었다.

해석은 아직 `약세 지속` 쪽이다. 현재 주가는 `5일선`, `20일선`, `60일선`, `120일선` 아래에 있고, 일목균형표 기준으로도 `below-cloud`다. RSI는 `35.61`로 과매도 직전이지만 추세 전환 신호는 아니다. `MACD` 역시 `시그널` 아래이면서 `0선` 밑에 있어 반등보다 약세 구조가 우위다.

다만 `히스토그램 축소`는 하락 가속이 커지는 구간은 아니라는 뜻이다. 반대로 `ADX 23.39`와 `-DI 우위`는 약세 추세 강도가 다시 붙는 구조라는 점을 보여준다. 아래로는 `149,300원` 이탈 여부가 가장 중요하고, 위로는 `153,350원` 전환선, `156,170원` 20일선, 그 다음 `168,200원` 돌파 레벨 회복이 필요하다. 차트만 놓고 보면 지금은 여전히 `선행 매수보다 확인 매수`가 더 맞는 자리다.
