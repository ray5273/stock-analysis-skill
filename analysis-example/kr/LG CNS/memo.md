...
- 현재 판단: `관심 유지, 추가 검증 전 보류`.
- 지금 가장 중요한 검증 포인트: `non-captive 매출 비중`, `데이터센터/AI 인프라 계약의 실제 수익성`, `분기 이익과 현금창출의 동행 여부`.
- 왜 아직 바로 못 사는가: LG전자와 LG화학 중심 고객 집중도, 아직 완전히 확인되지 않은 AI 인프라 economics, 그리고 차트상 `below-cloud` 구간이 동시에 남아 있다.
...
| --- | --- | --- | --- | --- |
| 2025년 실적은 매출 6조원대, 영업이익 5천억 후반대로 마감했다 | confirmed | 매출 `6조 1,295억`, 영업이익 `5,558억` | 2025년 4Q 실적발표 자료, 감사보고서 | 연간 연결 기준 |
| LG전자와 LG화학 중심의 고객 집중도가 여전히 높다 | confirmed | 2024년 기준 상위 2개 고객 합산 `50.1%` | 감사보고서 고객집중 공시 | 최신 공식 고객집중 공시는 2024년 기준 |
| 클라우드&AI가 실적 성장의 핵심 축이다 | partially supported | 클라우드&AI 매출 `3조 5,872억`, 전년 대비 `+7.0%` | 2025년 4Q 실적발표 자료 | 성장 확인, 다만 non-captive 비중은 별도 미공시 |
| 데이터센터와 AI 인프라 수주가 곧바로 높은 반복 매출로 이어진다 | needs follow-up | 계약과 보도는 확인되지만 반복 매출 구조와 수익성은 별도 미공시 | 회사 보도자료, IR 자료 | 내러티브는 강하지만 economics는 추가 검증 필요 |
...
`2026-04-03` 종가까지 반영한 최근 15거래일 PER/PBR 스냅샷은 아래 `## Update Log`의 `2026-04-04` 블록에 따로 붙였다.
...
![LG CNS 모멘텀 차트](../assets/LG CNS-chart-momentum.png)
...
차트 지표는 `종가 57,600원`, `5일선 57,680원`, `20일선 62,885원`, `60일선 66,502원`, `120일선 64,156원`, `볼린저 상단 70,353원`, `볼린저 하단 55,417원`, `전환선 60,300원`, `기준선 66,850원`, `현재 구름대 A 71,250원`, `현재 구름대 B 68,150원`, `미래 구름대 A 63,575원`, `미래 구름대 B 66,850원`, `RSI14 35.40`, `MACD -2,650`, `시그널 -1,993`, `히스토그램 -657`, `ADX14 20.22`, `+DI 16.07`, `-DI 29.97`, `거래량/20일 평균 49.3%`, `20일 돌파 레벨 69,800원`, `20일 이탈 레벨 55,400원`이었다.

차트 해석은 여전히 `약세 지속` 쪽이다. 현재 주가는 `20일선`, `60일선`, `120일선`과 `구름대` 아래에 있고, 일목균형표 기준으로도 `below-cloud`다. RSI는 `35.40`으로 과매도에 가까운 약세 중립 구간이며, `MACD`도 `시그널` 아래이면서 `0선` 밑에 있어 추세 전환 신호로 보긴 어렵다.

다만 `히스토그램 축소`는 하락 압력이 다시 강해지는 구간은 아니라는 뜻이다. 반대로 `ADX 20.22`와 `-DI 우위`는 약세 추세가 다시 힘을 받을 가능성까지 열어둬야 한다는 의미다. 아래로는 `55,417원~55,400원`대 방어 여부가 핵심이고, 위로는 `60,300원` 전환선, `62,885원` 20일선 회복이 첫 체크포인트다. 그 위에는 `68,150원~71,250원` 구름대가 더 강한 저항이다. 차트만 놓고 보면 현재 흐름은 `저점 통과 후 강한 추세 재개`가 아니라 `아직 약세 추세 안의 반등 대기 구간`이며, chart-only flow는 `bearish continuation`에 가깝다.
...
- LG CNS의 AI와 데이터센터 이야기는 실제로 `반복 매출 + 높은 수익성`으로 이어지는가, 아니면 일회성 구축 매출을 성장 서사로 포장한 것에 가까운가?
- `클라우드&AI` 성장의 얼마나 많은 부분이 진짜 non-captive 고객 확장이고, 얼마나 많은 부분이 계열사 내부 프로젝트 재분류인가?
- LG전자와 LG화학의 투자 사이클이 둔화되면 현재 시장이 기대하는 성장률은 얼마나 빨리 훼손되는가?
- 시장은 LG CNS에 `AI 인프라/플랫폼` 멀티플을 주고 있는데, 현재 filing이 그 정도의 사업 질을 실제로 증명하고 있는가?

## Decision-Changing Issues

1. `non-captive 매출 비중`이 구조적으로 올라가는지 여부.
2. 데이터센터와 AI 인프라 수주가 매출만이 아니라 마진과 현금흐름까지 끌어올리는지 여부.
3. 고객 집중도가 낮아지며 LG그룹 captive discount가 실제로 완화되는지 여부.
...
- `2026-04-04`는 토요일이라 이번 시각화 업데이트의 최신 장마감 기준일은 `2026-04-03`이다.
- `2026-04-03` 종가 `57,600원` 기준으로 LG CNS 메인 추세 차트, 오버레이 차트, 모멘텀 차트를 다시 생성했다. 메인 차트는 `캔들스틱 + 윗꼬리/아랫꼬리 + 종가선`, 한글 범례, 현재가 가이드 라인을 함께 표시하고, 모멘텀 차트는 `MACD + 시그널 + 히스토그램 + ADX/DMI`를 함께 보여주도록 바꿨다.
- 차트 지표는 `5일선 57,680원`, `20일선 62,885원`, `60일선 66,502원`, `120일선 64,156원`, `볼린저 상단 70,353원`, `볼린저 하단 55,417원`, `전환선 60,300원`, `기준선 66,850원`, `현재 구름대 A 71,250원`, `현재 구름대 B 68,150원`, `RSI14 35.40`, `MACD -2,650`, `시그널 -1,993`, `히스토그램 -657`, `ADX14 20.22`, `+DI 16.07`, `-DI 29.97`, `거래량/20일 평균 49.3%`였다.
- 해석은 `약세 지속` 쪽이지만, `2026-04-02` 대비 종가가 `56,500원 -> 57,600원`으로 반등했고 RSI도 `30.07 -> 35.40`으로 올라와 극단적 과매도 압력은 조금 완화됐다. 다만 `MACD`가 아직 `0선` 아래에서 `시그널` 밑에 있고, `ADX`도 `-DI` 우위로 서서히 올라오고 있어 `20일선`, `60일선`, `120일선`, `구름대` 아래라는 큰 약세 구조는 그대로다.

![LG CNS 2026-04-03 추세 차트](../assets/LG CNS-chart-2026-04-03.png)

![LG CNS 2026-04-03 오버레이 차트](../assets/LG CNS-chart-2026-04-03-overlay.png)

![LG CNS 2026-04-03 모멘텀 차트](../assets/LG CNS-chart-2026-04-03-momentum.png)

#### Recent PER/PBR Snapshot

이번 최근 PER/PBR 스냅샷은 `2025/12 EPS 4,549원`, `2025/12 BPS 30,270원`을 고정 기준으로 최근 `15거래일` 종가에 적용해 계산했다. 즉 `장기 밴드`가 아니라 `최신 결산 기준 멀티플이 최근 15거래일 안에서 어디쯤 내려왔는가`를 보는 용도다.

| Metric | Current | Min | Median | Max | Window |
| --- | --- | --- | --- | --- | --- |
| P/E | 12.7x | 12.3x | 13.7x | 14.9x | 최근 15거래일 |
| P/B | 1.9x | 1.8x | 2.1x | 2.2x | 최근 15거래일 |

```text
Recent P/E snapshot (2025/12 EPS base)
2026-03-16 | ####################### 14.4x
2026-03-17 | ######################## 14.6x
2026-03-18 | ######################## 14.7x
2026-03-19 | ####################### 14.3x
...
```text
Recent P/B snapshot (2025/12 BPS base)
2026-03-16 | ####################### 2.2x
2026-03-17 | ######################## 2.2x
2026-03-18 | ######################## 2.2x
2026-03-19 | ####################### 2.1x
...
- 최근 15거래일만 놓고 보면 `P/E 12.7x`, `P/B 1.9x`는 둘 다 구간 하단이다. 즉 현재 LG CNS는 `상장 초기 프리미엄이 아직 완전히 사라진 초고평가주`보다는 `최근 멀티플 디레이팅이 상당히 진행된 상태`로 읽는 편이 맞다.
